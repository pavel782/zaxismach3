...
namespace ModbusServer
{
    public class Utils
    {
        ...
        public static byte[] MakeCRC16(byte[] bytes, int count)
        { // CRC-16/MODBUS
            int icrc = 0xFFFF;
            ...
            byte[] ret = BitConverter.GetBytes(icrc).ToArray();

            return ret;
        }

        public static UInt16 ConvertBytesToUInt16(byte[] bytes)
        {
            return (UInt16)(bytes[0] * 256 + bytes[1]);
        }
